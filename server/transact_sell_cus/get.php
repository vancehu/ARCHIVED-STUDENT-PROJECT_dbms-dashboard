...
{
	$select =  'order_id, order_time, quantity, price, quantity*price AS total, transact_sell.cid, cname, transact_sell.pid, pname, transact_sell.eid, ename';
	$from = 'transact_sell, customer, employee, product';
	$cond = ' WHERE transact_sell.cid = customer.cid AND transact_sell.pid = product.pid AND transact_sell.eid = employee.eid AND customer.cid="'.$_SESSION['user']['info']['cid'].'"';
