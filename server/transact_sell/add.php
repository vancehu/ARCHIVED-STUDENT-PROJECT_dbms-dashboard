...
//query condition - MODIFY HERE ONLY
{
	...
	$cols = "order_id, order_time,";
	$values = "NULL, CURRENT_TIMESTAMP,";
	addInsert($cols, $values, $param, 'quantity');
	addInsert($cols, $values, $param, 'cid');
...
if(intval($check[0]['inventory_quantity'])<intval($param['quantity'])){
	echo '{"error":"2", "quant":"'.$check[0]['inventory_quantity'].'"}';
	exit;
}
