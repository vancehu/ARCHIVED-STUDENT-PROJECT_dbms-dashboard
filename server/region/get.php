...
{
	$select =  'region.rid, rname, region_manager.eid, ename';
	$from = 'region, region_manager, employee';
	$cond = ' WHERE region.rid = region_manager.rid AND region_manager.eid = employee.eid';
	addCondIs($cond, $param,  'rid', 'rid_not', 'region.rid');
	addCondLike($cond, $param, 'rname_like', 'rname_unlike', 'rname');
	addCondIs($cond, $param,  'eid', 'eid_not', 'region.eid');
}
...
if (isset($param['sort'])) {
	if($param['sort'] == "rid"){
		$param['sort'] = "region.rid";
	}
	if($param['sort'] == "eid"){
		$param['sort'] = "region_manager.eid";
	}
